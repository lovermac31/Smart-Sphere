import React, { useState } from 'react';
import { handleEnrollmentCheck } from '../services/enrollment';

export const EnrollmentForm = () => {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<any>(null);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    
    // Mock data based on the 9D Profile for the demo
    const mockStudent = {
      lexileScore: 1150,
      domains: { Thinking: 90, Listening: 80, Speaking: 75, Reading: 85, Writing: 70 },
      tier: 'Core'
    };

    const outcome = await handleEnrollmentCheck(mockStudent as any);
    setResult(outcome);
    setLoading(false);
  };

  return (
    <div className="p-6 max-w-md mx-auto bg-white rounded-xl shadow-md space-y-4">
      <h2 className="text-2xl font-bold text-gray-900">Summer Camp Interest Check</h2>
      <p className="text-gray-500">Discover your child's path to global universities like NYU or Oxford.</p>
      
      <form onSubmit={handleSubmit} className="space-y-4">
        <button 
          type="submit" 
          disabled={loading}
          className="w-full py-2 px-4 bg-blue-600 text-white font-semibold rounded-lg shadow-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-75"
        >
          {loading ? 'Analyzing...' : 'Check Readiness Now'}
        </button>
      </form>

      {result && (
        <div className="mt-4 p-4 bg-green-50 rounded-lg border border-green-200">
          <h3 className="font-bold text-green-800">Recommendation:</h3>
          <p className="text-green-700">{result.recommendation}</p>
          <p className="text-sm text-green-600 mt-2">Target Hub: {result.location}</p>
        </div>
      )}
    </div>
  );
};
